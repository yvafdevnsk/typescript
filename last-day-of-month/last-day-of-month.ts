 ...
 */
function init(): void {
    const showLastDayOfMonthButton: HTMLInputElement = document.getElementById("show-last-day-of-month-button") as HTMLInputElement;
 ...
 * @param e MouseEvent
 */
function showLastDayOfMonthButtonClickEventListener(e: MouseEvent): void {
    // 2023年n月の月末日を取得する。
    //
    ...
    const outputString: string = dateList
    .map((d) => {
    ...
    const leapYearOutputString: string = leapYearDateList
    .map((d) => {
        return d.toLocaleString("ja-JP", { year: "numeric", month: "2-digit", day: "2-digit" });
    })
    .join("\r\n");
    ...
    // 月末日を出力する。
    const logTextarea: HTMLTextAreaElement = document.getElementById("log-textarea") as HTMLTextAreaElement;
